...
JOIN (VALUES
  ...
  ('capacity-release', 'Что может снизить риск проблемного релиза?', 'Big bang deploy без метрик', 'Canary и rollback criteria', 'Отключение алертов', 'Релизы ночью без дежурных', 'B', 'Постепенный rollout и явные критерии отката — базовый паттерн надёжности.', 'https://sre.google/sre-book/reliable-product-launches/'),
